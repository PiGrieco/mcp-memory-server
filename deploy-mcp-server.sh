...
print_info "Step 2: Checking requirements.txt..."

if [ -f "requirements.txt" ]; then
    print_success "Using existing requirements.txt ($(grep -v "^#" requirements.txt | grep -v "^$" | wc -l) packages)"
    print_info "Not modifying requirements.txt - using your exact versions"
else
    print_error "No requirements.txt found! Please ensure requirements.txt exists with your working package versions."
    exit 1
fi
